...
The `@GetMapping` annotation indicates that `findPersonById` handles requests to the endpoint `GET /person/{pid}` (where `{pid}` should be replaced by an integer ID).
...
Similarly, the `@PostMapping` annotation indicates that `createPerson` handles requests to `POST /person`.
...
Using `curl`, we can then send a POST request to `/person` as follows, assuming our app is listening on port 8080.
```bash
curl --request POST 'http://localhost:8080/person' --data '{"name": "Alice", "email": "alice.allison@mail.mcgill.ca", "password": "password123"}' --header 'Content-Type: application/json'
```
...
```bash
curl --request GET 'http://localhost:8080/person/602'
```
